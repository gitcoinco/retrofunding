...
export type DistributeParams = {
  ...
  transactionData: { anchorAddress: Address; amount: bigint; index: number }[];
  poolId: string;
  chainId: number;
...
export const useDistribute = () => {
  ...
  const distributeMutation = useMutation({
    mutationFn: async ({
      ...
      const contractDistributionMetadata = (await retroFunding.getDistributionMetadata()) as any;
      ...
      const firstTime = contractDistributionMetadata[1] == "";
      const txData = retroFunding.distribute(
        transactionData.map((application) => getAddress(application.anchorAddress)),
        transactionData.map((application) => ({
